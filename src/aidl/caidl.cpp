...
char*
cfind_include_file(const char* given)
{
    ...
    vector<string>& paths = g_includePaths;
...
gather_types(const char* filename, document_item_type* items)
{
    ...
    while (items) {
        ...
        if (items->item_type == USER_DATA_TYPE) {
            user_data_type* p = (user_data_type*)items;
            type = new CUserDataType(p->package ? p->package : "", p->name.data,
                ...
                || items->item_type == INTERFACE_TYPE_RPC) {
            interface_type* c = (interface_type*)items;
            type = new CInterfaceType(c->package ? c->package : "",
        ...
        if (old == NULL) {
            ...
            if (items->item_type == INTERFACE_TYPE_BINDER) {
                ...
                name += ".Stub";
                CType* stub = new CType(c->package ? c->package : "",
                ...
                name = c->name.data;
                name += ".Stub.Proxy";
                CType* proxy = new CType(c->package ? c->package : "",
                                        name, CType::GENERATED, false, false, false,
                                        filename, c->name.lineno);
                CNAMES.Add(proxy);
            }
                ...
                string name = c->name.data;
                name += ".ServiceBase";
                CType* base = new CType(c->package ? c->package : "",
                                        name, CType::GENERATED, false, false, false,
                                        filename, c->name.lineno);
                CNAMES.Add(base);
            }
...
ccompile_aidl(Options& options)
{
    ...
    while (import) {
        if (CNAMES.Find(import->neededClass) == NULL) {
            import->filename = cfind_include_file(import->neededClass);
            if (!import->filename) {
    ...
    err = cgenerate_cpp(options.outputFileName, options.inputFileName.c_str(),
...
int
cpreprocess_aidl(const Options& options)
{
    vector<string> lines;
    int err;
